...
if __name__ == '__main__':
    ...
    images = [cv2.imread(file)
              for file in glob.glob(config['test_doc_image_path'])]
    answer_images = [cv2.imread(file)
                     for file in sorted(glob.glob(config['letter_answer_path']))]

    # execute OCR API; letter_info = x, y, width, height, centroid_x, centroid_y
    marked_image, letter_images, letter_info, sentence = ocr.OCRAPI_for_Web(
        images, answer_images, config['space_size'], config['newline_size'], config['detect_upper_letter'])
    ...
    for i in range(len(letter_info)):
        ...
        save_path_file = os.path.join(
            config['extract_image_path'], zero_str+str(i)+config['extract_img_format'])
